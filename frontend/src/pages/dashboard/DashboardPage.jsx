...
import {
  Package, Warehouse, Users, AlertTriangle,
  CheckCircle, XCircle, Activity, ArrowRight, TrendingUp,
} from 'lucide-react';
import { productsApi } from '../../api/productApi';
import { inventoryApi } from '../../api/inventoryApi';
import { usersApi } from '../../api/userApi';
import { userClient, productClient, inventoryClient } from '../../api/axiosConfig';
...
const SERVICE_DEFS = [
  { name: 'User Service', port: 3001, color: 'text-indigo-500', client: userClient },
  { name: 'Product Service', port: 3002, color: 'text-blue-500', client: productClient },
  { name: 'Inventory Service', port: 3003, color: 'text-green-500', client: inventoryClient },
  { name: 'Order Service', port: 3004, color: 'text-orange-400', client: null },
  { name: 'Payment Service', port: 3005, color: 'text-emerald-500', client: null },
  { name: 'Notification Service', port: 3006, color: 'text-purple-500', client: null },
];
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      const products = prodRes.status === 'fulfilled' ? prodRes.value.data : null;
      ...
      setStats({
        ...
        outOfStock: inv?.data?.filter((i) => i.stockStatus === 'OUT_OF_STOCK').length ?? '—',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard title="Inventory Items" value={stats?.totalInventoryItems} icon={Warehouse} color="blue" />
        <StatCard title="Low Stock Items" value={stats?.lowStockCount} icon={AlertTriangle} color="yellow" />
        <StatCard title="Out of Stock" value={stats?.outOfStock} icon={XCircle} color="red" />
        {isAdmin() && stats?.totalUsers !== null && (
          <StatCard title="Registered Users" value={stats?.totalUsers} icon={Users} color="green" />
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <Link to="/inventory?status=LOW_STOCK" className="text-xs text-indigo-600 flex items-center gap-1 hover:underline">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <Link to="/products" className="text-xs text-indigo-600 flex items-center gap-1 hover:underline">
              View all <ArrowRight size={12} />
            </Link>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div className="flex items-center gap-2 px-5 py-4 border-b border-gray-100">
          ...
          <h3 className="font-semibold text-gray-900 text-sm">Microservice Health</h3>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 divide-x divide-y sm:divide-y-0 divide-gray-100">
          {SERVICE_DEFS.map((svc) => {
            ...
            return (
              <div key={svc.name} className="flex flex-col items-center gap-2 px-4 py-4">
                <div
                ...
                <p className={`text-[10px] font-medium ${
                  ...
                  status === 'down' ? 'text-red-500' : 'text-gray-400'
                }`}>
                  {status === 'up' ? 'Online' : status === 'down' ? 'Offline' : 'Not deployed'}
                </p>
